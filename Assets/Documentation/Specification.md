...
| 6 | Rules & game logic | **Done** | `CatanBuildRule` (settlement/city/road placement with distance rule, road connection, resource checks), `CatanTradeRule` (phase + affordability + port ratio), `BuildManager`, `TradeManager`, `ScoreManager`, `LargestArmyTracker`, `LongestRoadTracker` (DFS), all dev card `IsPlayable`/`OnPlay`, `CatanGameContext`; EditMode tests written |
| 7 | Unity scene & UI | **Done** | `GameManager` (singleton, init, all game actions), `BoardRenderer` (hex+vertex+edge spawn, centroid-based positions), `HexTileView`, `VertexView`, `EdgeView` (click handlers), `PlayerHandView` (resource counts per player), `TurnIndicatorView` (phase/turn labels), `ActionButtonsView` (phase-gated buttons), `RobberView` (world-space icon), `VictoryScreenView` (win overlay) |
...
| 2026-05-10 | Phase 6: implemented CatanBuildRule (settlement distance rule/road connection/resource checks, city upgrade, road placement, always-false CanRemove), CatanTradeRule (phase gate, bank ratio check via ports, player affordability), BuildManager (TryPlace/TryRemove with events), TradeManager (propose/accept/reject/cancel with status tracking), ScoreManager (GetScore/GetLeader/CheckVictory/RecalculateAll), LargestArmyTracker (3-knight minimum, strict beat), LongestRoadTracker (DFS with opponent-settlement blocking, 5-road minimum), all dev card IsPlayable/OnPlay (Knight/Monopoly/YearOfPlenty/RoadBuilding/VictoryPoint), CatanGameContext; wrote EditMode tests for CatanBuildRule, LargestArmyTracker, LongestRoadTracker, DevCards, ScoreManager |
| 2026-05-10 | Phase 7: implemented GameManager (MonoBehaviour singleton, full init sequence, all game actions: TryPlaceSettlement/Road/City/MoveRobber, TryPurchaseDevCard, TryPlayDevCard), BoardRenderer (spawns HexTileView/VertexView/EdgeView from prefabs, centroid-based vertex positions from HexVertex.AdjacentTiles), HexTileView (tile sprite + number token + robber icon toggle), VertexView (settlement/city sprite on click), EdgeView (road sprite on click), PlayerHandView (per-player resource counts + VP, subscribes to ResourceAddedEvent/ResourceRemovedEvent), TurnIndicatorView (active player + phase label + turn number), ActionButtonsView (Roll/EndTurn/Build buttons enabled per phase and affordability), RobberView (world-space icon teleports on RobberMovedEvent), VictoryScreenView (subscribes to VictoryAchievedEvent, shows winner + restart button) |
